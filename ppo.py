...
if __name__ == "__main__":
    ...
    envs = [Doors(max_steps=args.num_steps,seed=args.seed + i) for i in range(args.num_envs)]
    ...
    next_obs = torch.Tensor([env.reset() for env in envs]).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            actions[step] = torch.functional.F.one_hot(action,num_classes=envs[0].single_action_space[0])
            logprobs[step] = logprob

            # TRY NOT TO MODIFY: execute the game and log data.
            #next_obs, reward, done, info = envs.step(action.cpu().numpy())
            next_obs, reward, done, info = step_all(envs,action.cpu().numpy())
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds], b_actions.long()[mb_inds].argmax(dim=1))
    ...
    torch.save(agent,f'ppo_agent_iter_{args.total_timesteps}_mlp.pth')